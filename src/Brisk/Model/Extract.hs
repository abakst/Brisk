...
import           Brisk.Model.IceT (runIceT)
import           Brisk.Pretty
...
type Annot  = SrcSpan
type AbsEff = EffExpr Id Annot
type EffMap = Env.Env Id (EffExpr Id Annot)
...
notLocated :: Functor f => f a -> f SrcSpan             
notLocated = locate noSrcSpan

locate :: Functor f => SrcSpan -> f a -> f SrcSpan
locate s = fmap (const s)
...
runMGen bs hsenv mg specs prog
  = do initBinds <- resolve hsenv (specTuple <$> specs)
       let g0    = Env.addsEnv Env.empty [ (nameId x, notLocated b) | (x,b) <- initBinds ]
...
mGenBind g (Rec [(f,e)])
  = do let g' = Env.insert g n guess
       a <- mGenExpr g' e
       return (Env.insert g n (ERec n a (getSrcSpan f)))
         where
           (bs,_)  = collectBinders e
           bes     = [ EVar (bindId x) (getSrcSpan x) | x <- bs, not (isDictId x) ]
           guess   = foldr go (foldl go' (var n (getSrcSpan f)) bes) bes
           n       = bindId f
           go (EVar x l) a  = ELam x a l
           go' a x = EApp a x (getSrcSpan f)
mGenBind g (Rec bs)
...
mGenExpr g (Type t)
  = do s <- currentSpan
       return (EType (ofType bindId t) s)
mGenExpr g (Cast e _)
  = mGenExpr g e
...
mGenExpr g (Var x)
  ...
  = return $ conEffExpr (getSrcSpan x) (dataConOrigResTy dc) dc

mGenExpr g (Var x)
  | Just a <- Env.lookup g (bindId x)
  = return a
  | otherwise
  = return $ var (bindId x) (getSrcSpan x)
...
mGenExpr g (Lam b e)
  = do a <- mGenExpr (Env.insert g n (var n $ getSrcSpan b)) e
       s <- currentSpan
       if isDictId b then
         return a
       else
...
mGenExpr g e@(Case e' _ t alts)
  = do a     <- mGenExpr g e'
       ...
       s     <- currentSpan
       let tE = ofType bindId (exprType e')
       return $ ECase tE a as defA s
         where
...
mGenMonadOp f tc
  = do tc' <- gets procTy
       if tc' == getName tc then
         return (notLocated <$> monadOp f)
       else
         return Nothing

mGenApp :: EffMap -> AbsEff -> AbsEff -> MGen AbsEff
mGenApp g e@(ECon _ _ _ _) (EType _ _)
  = return e
mGenApp g e@(ECon _ _ _ _) a
  = return (e `apConEff` a)
mGenApp g a@(ELam _ _ _) a2
...
mGenCaseAlts g = mapM go
  where
    go (DataAlt c, bs, e)
      = do let g' = Env.addsEnv g [ (bindId b, var (bindId b) (getSrcSpan b)) | b <- bs ]
           a <- mGenExpr g' e
...
litEffect l (LitInteger i _) = EVal (pInt vv (fromInteger i) l) l
litEffect l (MachInt i)      = EVal (pInt vv (fromInteger i) l) l
litEffect l (MachInt64 i)    = EVal (pInt vv (fromInteger i) l) l
-- litEffect l _ = EVal (vv, PTrue) l
